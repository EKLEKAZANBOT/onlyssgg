...
client.on("ready", () => {
  client.channels.get("753976438171893811").join();
   //main dosyaya atılacak
})

client.on("guildMemberAdd", async member => {
  if (member.guild.id !== "753026692699783168") return;
  let channel = client.channels.get("756495724002148382");
  channel.setName("Son Üye: " + member.user.username);
});

client.on("message", msg => {
  var dm = client.channels.get("755070953788145794"); //mesajın geleceği kanal idsi//
